"""Interactive pentode (6AU6) COMMON-CATHODE AMPLIFIER for Blender 5.x.

Build standalone:   blender -P pentode_amp_sim.py
Or from a console:  import pentode_amp_sim; pentode_amp_sim.build_all()

The verified 6AU6 pentode simulation wired into the classic RC stage with
TWO solved load lines:

    B+ --[ plate resistor R_L 20k-500k ]-------> plate
    B+ --[ SCREEN resistor R_g2 50k-1M ]-------> screen grid g2
...
No plate or screen-voltage sliders: each frame both Vp = B+ - Ip*R_L and
Vg2 = B+ - Ig2*R_g2 are solved by nested bisection against a calibrated
companion model of the tube (perveance K and screen fraction S calibrated
live from the measured electron current). Unlike the triode stage, gain
here has no mu ceiling -- it scales with R_L. Uncheck "Screen bypass" to
see why bypass capacitors exist; uncheck "Suppressor connected" for the
tetrode distortion demo.

Open the "Pentode Amp" tab in the sidebar (N key), press Run / Pause, drag
sliders live. Stateful sim: use Reset, don't scrub the timeline.
"""
import math
...
def _upd_rl(self, context):
    _recolor_bands("RL", self.pamp_rl)


def _upd_rg2(self, context):
    _recolor_bands("RG2", self.pamp_rg2)


def _upd_bplus(self, context):
    ob = _ob("PSUVal")
    if ob:
        ob.data.body = f"{self.pamp_bplus:.0f} V"
    # OUT channel auto-range: 0-300 V below 300 V B+, 0-500 V above
    vmax = 300 if self.pamp_bplus <= 300.0 else 500
...
def _push_traces():
    ...
    bp = float(getattr(_scene(), "pamp_bplus", 300.0))
...
def _apply_heat(scene=None):
    sc = scene or _scene()
    T = getattr(sc, "pamp_heater_t", T_REF)
    color, x = _glow(T)
...
def reset_electrons():
    ...
    _S["last_txt"] = ""
    _S["t"] = 0.0
    _S["vp"] = float(getattr(_scene(), "pamp_bplus", 300.0))   # tube off ->
...
def _step(scene):
    ...
    if "pos" not in S:
        reset_electrons()
    rng = S["rng"]
    T = scene.pamp_heater_t
    sup = scene.pamp_suppressor
    ...
    S["t"] += DT
    Vg1 = scene.pamp_sig_dc + scene.pamp_sig_amp * math.sin(
        2 * math.pi * FREQ * S["t"])
    Bp = scene.pamp_bplus
    RL = scene.pamp_rl
    RG2 = scene.pamp_rg2
...
def amp_pentode_frame_change(scene, depsgraph=None):
    try:
        _step(scene)
        _S["fails"] = 0
    except Exception:
        ...
        if _S["fails"] > 5:
            _remove_handlers()
            print("pentode_amp_sim: handler removed after repeated errors")
...
def register_sim():
    _remove_handlers()
    bpy.app.handlers.frame_change_pre.append(amp_pentode_frame_change)
    if "pos" not in _S:
        reset_electrons()
...
def _upd_glass(self, context):
    g = _ob("Glass")
    if g:
        g.hide_viewport = not self.pamp_show_glass
        g.hide_render = g.hide_viewport


def _upd_suppressor(self, context):
    show = self.pamp_suppressor
    for nm in ("Grid3", "Grid3RodA", "Grid3RodB"):
        ob = _ob(nm)
        if ob:
            ob.hide_viewport = not show
...
class PENTAMP_OT_reset(bpy.types.Operator):
    bl_idname = "pentamp.reset"
    bl_label = "Reset"
...
class PENTAMP_OT_play(bpy.types.Operator):
    bl_idname = "pentamp.play"
    bl_label = "Run / Pause"
...
class PENTAMP_OT_view(bpy.types.Operator):
    bl_idname = "pentamp.view"
    bl_label = "View"
...
class PENTAMP_PT_main(bpy.types.Panel):
    bl_label = "Pentode Amp"
    bl_idname = "PENTAMP_PT_main"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = "Pentode Amp"

    def draw(self, context):
        s = context.scene
        L = self.layout
        col = L.column(align=True)
        col.prop(s, "pamp_heater_t", slider=True)
        col.prop(s, "pamp_bplus", slider=True)
        col.prop(s, "pamp_rl", slider=True)
        col.prop(s, "pamp_rg2", slider=True)
        col.separator()
        col.prop(s, "pamp_sig_amp", slider=True)
        col.prop(s, "pamp_sig_dc", slider=True)
        L.prop(s, "pamp_g2_bypass")
        L.prop(s, "pamp_suppressor")
        row = L.row(align=True)
        row.operator("pentamp.play", icon='PLAY')
        row.operator("pentamp.reset", icon='FILE_REFRESH')
        row = L.row(align=True)
        for label, key in (("Bench", "OVER"), ("Top", "TOP"), ("Inside", "INSIDE")):
            row.operator("pentamp.view", text=label).which = key
        L.prop(s, "pamp_show_glass")
        box = L.box()
...
def register_ui():
    Sc = bpy.types.Scene
    for name in ("pamp_heater_t", "pamp_bplus", "pamp_rl", "pamp_rg2",
                 "pamp_sig_amp", "pamp_sig_dc", "pamp_g2_bypass",
                 "pamp_suppressor", "pamp_show_glass"):
        if hasattr(Sc, name):
            try:
                delattr(Sc, name)
            except Exception:
                pass
    Sc.pamp_heater_t = bpy.props.FloatProperty(
    ...
    Sc.pamp_bplus = bpy.props.FloatProperty(
    ...
    Sc.pamp_rl = bpy.props.FloatProperty(
    ...
    Sc.pamp_rg2 = bpy.props.FloatProperty(
    ...
    Sc.pamp_sig_amp = bpy.props.FloatProperty(
        name="Signal amplitude (Vpk)", min=0.0, max=8.0, default=0.5,
        step=10, precision=1,
        description="Sine on g1; pentode gain is high, start small")
    Sc.pamp_sig_dc = bpy.props.FloatProperty(
        name="Grid bias / DC offset (V)", min=-15.0, max=5.0, default=-3.0,
        step=10, precision=1,
        description="Generator DC offset = g1 bias; sets the operating point")
    Sc.pamp_g2_bypass = bpy.props.BoolProperty(
    ...
    Sc.pamp_suppressor = bpy.props.BoolProperty(
    ...
    Sc.pamp_show_glass = bpy.props.BoolProperty(
        name="Show glass envelope", default=True, update=_upd_glass)

    for cls_name in ("PENTAMP_OT_reset", "PENTAMP_OT_play",
                     "PENTAMP_OT_view", "PENTAMP_PT_main"):
        ...
        if old is not None:
            try:
                ...
                pass
    for cls in (PENTAMP_OT_reset, PENTAMP_OT_play, PENTAMP_OT_view, PENTAMP_PT_main):
...
def selfcheck():
    ...
    def run(T, dc, A, bp, rl, rg2, frames, sup=True, byp=True):
        sc.pamp_heater_t, sc.pamp_sig_dc, sc.pamp_sig_amp = T, dc, A
        sc.pamp_bplus, sc.pamp_rl, sc.pamp_rg2 = bp, rl, rg2
        sc.pamp_suppressor, sc.pamp_g2_bypass = sup, byp
        reset_electrons()
    ...
    # The screen resistor self-limits Ig2, so the visible signature is the
    # ORANGE SECONDARIES IN FLIGHT (escaping plate->screen), not the meter.
    sec_pen = int(np.count_nonzero(
        run(1100, -3, 1.0, 300, 300, 150, 380)["alive2"]))
    sec_tet = int(np.count_nonzero(
        run(1100, -3, 1.0, 300, 300, 150, 380, sup=False)["alive2"]))
    assert sec_tet > 1.4 * max(sec_pen, 10), (   # tetrode distortion demo
        f"tetrode mode should free secondaries: {sec_tet} !> 1.4x{sec_pen}")
    ...
    print(f"selfcheck OK  op Vp={vp0:.0f}V Vg2={vg20:.0f}V | "
          ...
          f"unbypassed {g_unbyp:.1f}x | kink secondaries {sec_pen}->{sec_tet}")
    return True


if __name__ == "__main__":
    # Fresh scene (blender -P pentode_amp_sim.py): full build.
